...
### AI-Mentor - ЗАВЕРШЕН (Этап 2 с API интеграцией)
**Статус:** Полностью реализован с OpenAI GPT-4o
**URL:** `/ai-mentor`
**Описание:** AI-ментор для предпринимателей с выбором архетипов

**Реализованные функции:**
- 6 менторов (3 активных с API + 3 coming-soon)
- 3-экранный флоу: выбор → проблема → результат
- **OpenAI GPT-4o интеграция** с токен-лимитами
...
**Менторы:**
- **Активные с API:** Стив Джобс, Далай-лама XIV, Альберт Эйнштейн
- **Coming Soon:** Билл Гейтс, Олег Тиньков, Сергей Галицкий

**API архитектура:**
- Server Actions для безопасности API ключей
- Промпт-инжиниринг с персонализацией под каждого ментора
- Токен-лимиты: 4096 input, 2048 output
...
│       │   ├── ClarificationBlock.tsx # Блок уточнений
│       │   └── SolutionBlock.tsx    # Блок решения
│       ├── lib/              # Утилиты
...
├── ai-mentor/               # Данные AI-ментора (fallback)
│   └── data/
│       ├── mentors.json     # 6 менторов
│       └── mock-responses.json  # Fallback ответы
└── images/
...
- `ResultBlock.tsx` - результат (4 состояния)
- `ClarificationBlock.tsx` - блок уточнений от AI
...
- **Этап 2:** AI-Mentor (OpenAI API интеграция) - ✅ ЗАВЕРШЕН
- **Этап 3:** AI-Mentor (дизайн полировка) - 🔄 В процессе
- **Этап 4:** Дополнительные AI-инструменты - В планах
...
- Не коммитить API ключи в код
- Server Actions для безопасного API доступа
...
Полная техническая документация по инструментам:
- `docs/tools/ai-mentor/` - детальная архитектура, дизайн, README
